<img src="imgs/logo.png" width=100% height="400px">

# 使用方法

### 安装依赖

1. 确保你已经安装了 Rust 编译器和 `cargo`
2. 为了编译rust为wasm，安装[wasm pack工具包](https://rustwasm.github.io/wasm-pack/installer/)
3. 对于前端的环境，请确保安装了nodejs、npm和vue
...
- Pattern管理：
  ...
  - 左键在对应的pattern上按下，可以上下拖拽pattern进行排序
  - `<u>`选中pattern ctrl+C弹出确认窗口，回车确认后复制pattern
  - 琶音器 `</u>`
...
- 播放单元
  ...
  - 播放单元管理播放歌曲的哪个部分，以及从什么时候开始播放
    ...
    - 设置bpm
    - `<u>`更改播放逻辑，不是先导出采样后播放而是在缓冲区中实时写入数字音频文件并播放 `</u>`
